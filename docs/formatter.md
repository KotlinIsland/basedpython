...
- [`multiple-spaces-before-operator`](rules/multiple-spaces-before-operator.md) (`E221`) if used with
    [`assignment-alignment`](settings.md#assignment-alignment) enabled, which lines the `=` of
    consecutive assignments up by putting more than one space in front of the operator
