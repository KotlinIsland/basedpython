...
pub enum FrameworkRole {
    ...
    SqlalchemyDeclarative,  // lands with the sqlalchemy session
    DjangoModel,            // lands with the django session
}
...
- salsa-tracked, mro-based, delegating to each dedicated module's detection
    function (`pydantic::is_model` today; `sqlalchemy::is_declarative`,
    `django::is_model` when those sessions land)
...
- **descriptor-annotated fields** (sqlalchemy) — the annotation is a marker
    generic: `x: Mapped[int]` declares a field whose instance type is `int`
    and whose class-level type is the descriptor's `__get__` result
    (`InstrumentedAttribute[int]`). the engine unwraps the marker; which
    classes count as markers is the `FieldKind` arm's decision
- **value-inferred fields** (django) — the field is an *unannotated*
    assignment `name = CharField(max_length=100)` whose RHS is a descriptor
    instance; field types come from the descriptor's `__get__`/`__set__`
    (which django-stubs encodes in the field's generic parameters)
...
| based enums as field types                        | ✓ — variants lower to stdlib dataclasses, which pydantic validates natively                                                                                                                       | needs `TypeDecorator`; ✓ to declare, D later on column use                   | needs custom field; same                                                                                                                                                                     | ✓                                                                                                    |
| reified generics / type reification               | ✓ — generic models implement `__class_getitem__`; constructor reification `M[int](...)` is native pydantic. the `@generic` wrapper must never be applied to a model class (G in the reified pass) | G — models are rarely generic; reification skips non-generic classes already | G                                                                                                                                                                                            | ✓                                                                                                    |
...
| optional chaining / force unwrap / coalesce       | ✓                                                                                                                                                                                                 | ✓ — useful on nullable relationships                                         | ✓                                                                                                                                                                                            | ✓                                                                                                    |
| lazy imports                                      | ✓                                                                                                                                                                                                 | ✓                                                                            | note — django relies on import side effects for model/signal registration; lazy lowering already materializes on first attribute access, but the django session must add divergence coverage | ✓                                                                                                    |
