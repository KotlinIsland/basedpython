...
| ----------------------------------------------- | ------------------------------------ | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| monkey-patching a module function               | works                                | works, unless the project binds early | a compiled call asks whether the name still holds the function; see [runtime](runtime.md#a-module-function-rebound-from-outside) |
...
| a `float` subclass read into a `float` place    | keeps its type and its methods       | narrowed to its value                 | a double has nothing to point at; see [runtime](runtime.md#a-float-subclass-read-into-a-place-declared-float)                    |
...
| stack depth on deep recursion                   | python's limit                       | python's limit, and the C stack's     | a native frame lives on the stack; see [runtime](runtime.md#how-deep-a-recursion-goes)                                           |
