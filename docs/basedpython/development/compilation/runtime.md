...
python 3.11 has no watchers, and a free-threaded build registers none, so there each
compiled call reads the name and compares the function's `__code__`, `__defaults__` and
`__kwdefaults__` with the objects it was published with. that is a lookup per call, and a
free-threaded build asks it on every trip round a loop as well, since another thread can
rebind the name while the loop runs. the answers are the watchers' but for one shape: a
function given back the very dict it was published with finds it standing again, where the
watchers keep it moved, so an edit made inside that dict meanwhile is not seen:

```python
original = mod.scaled.__kwdefaults__
mod.scaled.__kwdefaults__ = {"scale": 3}
original["scale"] = 10
mod.scaled.__kwdefaults__ = original
mod.scaled(2)  # python: 20; compiled with watchers: 20; compiled without: 2
```
