    ...
    fn visit_stmt(&mut self, stmt: &'a Stmt) {
        ...
        match stmt {
            Stmt::FunctionDef(
                ...
                // (`def f() raises ValueError`). nothing here visits it, so without this
                ...
                // the cost is that `TC001`-`TC003` never offer to move a raises-only
                // import, which is the safe direction to be wrong in
            ...
            Stmt::ClassDef(
                ...
                self.semantic.pop_definition();
                self.semantic.pop_scope(); // Type parameter scope
                // basedpython: an `extension T:` header *references* the type it
                // names instead of declaring it. binding the header name would read
                // as a redefinition of the real declaration
                if class_def.is_extension() {
