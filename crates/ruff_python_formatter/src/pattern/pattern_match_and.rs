...
use ruff_python_ast::PatternMatchAnd;
...
#[derive(Default)]
pub struct FormatPatternMatchAnd;

impl FormatNodeRule<PatternMatchAnd> for FormatPatternMatchAnd {
    fn fmt_fields(&self, item: &PatternMatchAnd, f: &mut PyFormatter) -> FormatResult<()> {
        let PatternMatchAnd {
            range: _,
            node_index: _,
            patterns,
        ...
        let inner = format_with(|f: &mut PyFormatter| {
            ...
            for pattern in patterns {
                ...
                write!(f, [token("and"), space(), pattern.format()])?;
            }
...
impl NeedsParentheses for PatternMatchAnd {
