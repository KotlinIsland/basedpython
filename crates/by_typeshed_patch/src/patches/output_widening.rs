...
//! returns a *fresh* container of the same class — `list.copy`, `dict.copy`, the
//! set algebra (`union`, `difference`, ...) — hands back a brand new object the
//! caller solely owns, so widening its element type at the call site is sound
//!
...
//!
//! only methods reached by an ordinary call are widened, never operator dunders
//! (`__add__`, `__getitem__`, ...): a call already threads the caller's expected
//! type into inference, whereas widening an operator's return would need
//! bidirectional inference on every binary op / subscript, which is far too
//! expensive on real code. see `is_dunder`
//!
...
fn widen_method(
    ...
    // only widen methods reached by an ordinary call (`copy`, `union`,
    // `difference`, ...), never operator dunders (`__add__`, `__getitem__`, ...).
    // a call already threads the caller's expected type into inference, so the
    // `Never`-defaulted parameter solves for free; an operator's expected type is
    // not, and making it so requires bidirectional inference on every binary op
    // and subscript — far too expensive on real code (it times out projects like
    // xarray). on an operator the parameter would just be dead weight
    if is_dunder(func.name.as_str()) {
        return;
    }
...
/// whether `name` is a dunder such as `__add__` or `__getitem__`
...
";
    ...
    fn extends_existing_type_params() {
        // `union` already carries a method type parameter inferred from its argument; the widening
        // parameter is appended after it
        let src = "\
from typing_extensions import Never
class set[in out Element]:
...
";
    ...
    #[test]
    fn leaves_operator_dunders_unwidened() {
        // operator dunders can only widen through bidirectional inference on the operator, which is
        // deliberately not attempted, so they are left alone even though they return a fresh `list`
        let src = "\
class list[in out Element]:
    def __add__[Other](self, value: list[Other], /) -> list[Other | Element]: ...
    def __mul__(self, value: SupportsIndex, /) -> list[Element]: ...
...
class dict[in out Key, in out Value]:
    if sys.version_info >= (3, 9):
        def merged(self, other: dict[Key, Value], /) -> dict[Key, Value]: ...
...
class dict[in out Key, in out Value]:
    if sys.version_info >= (3, 9):
        def merged[WidenKey = Never, WidenValue = Never](self, other: dict[Key, Value], /) -> dict[Key | WidenKey, Value | WidenValue]: ...
    class _NestedView[in out Element]:
        def copy(self) -> _NestedView[Element]: ...
";
        assert_eq!(run("builtins.byi", src), expected);
