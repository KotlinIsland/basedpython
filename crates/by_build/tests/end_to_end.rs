...
fn built(module: &ModuleIr, toolchain: &Toolchain, tag: &str) -> Option<Scratch> {
    let dir = Scratch::new(format!("by_e2e_{tag}"));
...
fn floats_are_unboxed_and_exclude_int() {
    ...
    assert_eq!(out, "type soundness violation: expected float, got int");
}
...
fn annotate_writes_a_report_beside_the_generated_c() {
    ...
    let dir = Scratch::new("by_e2e_annotate");
    let source = "\
...
fn an_accumulated_string_is_grown_rather_than_copied() {
    ...
    let dir = Scratch::new("by_e2e_append");
...
fn no_annotation_is_written_unless_it_is_asked_for() {
    ...
    let dir = Scratch::new("by_e2e_noannotate");
    let Ok(built) = build_source(
        "def f(a: int) -> int:\n    return a\n",
        "by_e2e_noannotate",
        &toolchain,
...
fn the_fallback_honours_a_supplied_transpiler_config() {
    ...
    let dir = Scratch::new("by_e2e_fallbackconfig");
...
";
    ...
    let plain = Scratch::new("by_e2e_fallbackplain");
...
fn a_stdlib_module_compiles_without_a_hard_failure(name: &str) {
    ...
    let dir = Scratch::new(format!("by_e2e_corpus_{module}"));
...
fn a_caller_supplied_lowering_is_compiled_the_same_way() {
    ...
    let dir = Scratch::new("by_e2e_lowered");
    let source = "\
...
fn require_native_still_bites_on_a_caller_supplied_lowering() {
    ...
    let dir = Scratch::new("by_e2e_loweredstrict");
...
fn an_unchanged_module_is_not_recompiled() {
    ...
    let dir = Scratch::new("by_e2e_rebuild");
...
fn a_stale_extension_is_rebuilt_even_when_the_c_is_unchanged() {
    ...
    let dir = Scratch::new("by_e2e_rebuild_stale");
    let source = "def double(a: int) -> int:\n    return a * 2\n";
    let Ok(built) = build_source(
        source,
...
fn the_emitted_c_names_no_pointer_type_it_does_not_mean() {
    ...
    let dir = Scratch::new("by_e2e_pointers");
    let built = by_build::emit_source(
        POINTER_SOURCE,
        "by_e2e_pointers",
        None,
...
fn a_package_is_built_as_a_tree_and_imports_under_its_dotted_names() {
    ...
    let dir = Scratch::new("by_e2e_package_tree");
...
fn running_the_interpreter_with_o_still_means_o_for_the_twin() {
    ...
    let dir = Scratch::new("by_e2e_twinoptrun");
    let source = "\
try:
...
fn running_the_interpreter_with_oo_takes_the_docstrings_out_of_the_compiled_definitions() {
    ...
    let dir = Scratch::new("by_e2e_docoptrun");
    let source = "\
def plain():
    \"the plain one\"
    return 1
...
fn the_running_version_is_read_off_the_banner() {
    ...
    let dir = Scratch::new("by_e2e_version");
