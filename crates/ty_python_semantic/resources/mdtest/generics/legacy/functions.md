...
## Container constraints infer the element type

basedpython's typeshed declares `Collection`'s parameter covariant, so it can inherit
`Container[Element]` rather than the `Container[Any]` upstream needs to keep covariance. Inferring a
type variable from a collection passed to a `Container` therefore lands on the element type itself,
with no gradual constraint left to preserve.
...
items: list[str] = []
reveal_type(value(items))  # revealed: str
```
