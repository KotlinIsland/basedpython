...
# a dict is not `Hashable`, so a dict keyed by itself violates `dict`'s key bound
RecursiveDict3 = dict["RecursiveDict3", int]  # error: [invalid-type-arguments]
...
def _(
    ...
    reveal_type(recursive_dict3)  # revealed: dict[Unknown, int]
