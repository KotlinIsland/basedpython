...
# `item` is a mutable public attribute, so the class cannot be covariant in its type
...
def builtin_positional_patterns_are_exhaustive(
    value: tuple[
        ...
        dict[object, object],
        float,
        # basedpython's typeshed bounds a `frozenset`'s element by `Hashable`, which `object` is
        # not, so the widest `frozenset` here is one over a hashable element
        frozenset[str],
        int,
        list[object],
        set[object],
