...
@overload
@decorate_overload
# basedpython also checks each overload against the implementation, and a decorator that
# rewrites one side's signature but not the other's makes them disagree
# error: [invalid-overload]
def decorated() -> None: ...
@overload
