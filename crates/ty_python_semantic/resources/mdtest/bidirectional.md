...
reveal_type(x9)  # revealed: dict[Literal["$in"], list[str]]
```
...
reveal_type(x7)  # revealed: list[int | str]
```
...
reveal_type(x2)  # revealed: tuple[list[Literal[1]], list[Literal[1]], list[Literal[1]]]
...
# with the fork's covariant `Mapping` key, the narrower inferred key type already
# satisfies the annotation, so it is not widened to the declared `Hashable`
x20: Mapping[Hashable, list[Callable[..., Any]]] = {"x": [variadic]}
reveal_type(x20)  # revealed: dict[str, list[(...) -> Any]]

x21: Mapping[Hashable, list[Callable[..., Any]]] = dict(x=[variadic])
reveal_type(x21)  # revealed: dict[str, list[(...) -> Any]]
...
# error: [invalid-argument-type] "Argument to class `A` is incorrect: Expected `list[int | str]`, found `list[list[Unknown]]`"
A(f([]))
```
...
reveal_type(f9)  # revealed: (*args, *, x: int = 1) -> None
...
def _(x: bool):
    ...
    # `signatures` is a fluid specialization candidate, so this statement is re-inferred
    # with the receiver's identity specialization; the lambda picks up a parameter
    # annotation from that pass. `f` itself is unaffected
    # revealed: (x: Unknown) -> Unknown
