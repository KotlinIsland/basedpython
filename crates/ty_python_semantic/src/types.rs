...
impl<'db> Type<'db> {
    ...
    fn to_meta_type(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> Type<'db> {
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).to_meta_type(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).to_meta_type(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).to_meta_type(db, env),
