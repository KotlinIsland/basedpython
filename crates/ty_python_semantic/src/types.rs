...
pub use self::dedicated::pytest::{
    CollectedTest, CollectedTestClass, FixtureBinding, FixtureExposure, FixtureNameSource,
    collected_pytest_tests, fixture_bindings_for_parameter, fixture_exposures_for_definition,
...
pub(crate) use self::match_pattern::{
    ...
    class_pattern_positional_result, class_pattern_positional_sources, definite_match_pattern_type,
...
pub(crate) use class::{
    ClassLiteral, ClassLiteralFlags, ClassType, GenericAlias, StaticClassLiteral,
};
...
pub(crate) fn check_types_with(
    ...
    report_external(db, source_file, external, &mut diagnostics);
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        let instance_class = match self.instance_class(db) {
            PropertyInstanceClass::Subclass(class) => PropertyInstanceClass::Subclass(
                class.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
            ),
            class => class,
        };
...
pub enum Type<'db> {
    ...
    /// behaves like its reduced form (see [`DeferredType`]) everywhere else.
...
impl<'db> GeneratorTypes<'db> {
    ...
    fn materialize(
        ...
        Self {
            yield_ty: self
                .yield_ty
                .map(|ty| ty.materialize(db, env, kind, &visitor)),
            send_ty: self
                .send_ty
                .map(|ty| ty.materialize(db, env, kind.flip(), &visitor)),
            return_ty: self
                .return_ty
                .map(|ty| ty.materialize(db, env, kind, &visitor)),
        }
...
fn nesting_depth<'db>(
    ...
    if let Type::Union(union) = ty {
        return union
            .elements(db)
            .iter()
            .map(|element| nesting_depth(db, env, *element, limit))
            .max()
            .unwrap_or(0);
    }
    ...
    let argument = match instance.tuple_spec(db, env) {
        Some(spec) => spec.homogeneous_element_type(db, env),
        None => match instance
            .class(db, env)
            .class_literal_and_specialization(db)
            .1
        {
            Some(specialization) => UnionType::from_elements(db, env, specialization.types(db)),
            None => return 1,
        },
    };
    1 + nesting_depth(db, env, argument, limit - 1)
}
...
impl<'db> Type<'db> {
    ...
    /// down.
    fn without_growing_tuple_lengths(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        previous: Self,
        ...
        rest.push(Type::homogeneous_tuple(
            db,
            env,
            UnionType::from_elements_cycle_recovery(db, env, elements),
        ));
        UnionType::from_elements_cycle_recovery(db, env, rest)
    }
    ...
    fn collapse_tuple_lengths(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> Self {
        ...
        kept.push(Type::tuple(crate::types::tuple::TupleType::homogeneous(
            db, env, element,
        )));
    ...
    fn has_gradual_member_impl(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        descend_typevars: bool,
    ) -> bool {
        let gradual = |element: Type<'db>| {
            element.is_dynamic() || element.has_gradual_member_impl(db, env, descend_typevars)
        };
        match self {
    ...
    pub fn has_dynamic(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> bool {
    ...
    fn protocol_data_members(
        ...
        Some(
            protocol
                ...
                .into_iter()
                .filter_map(|member| match member.reified_member_shape(db, env)? {
    ...
    pub fn unpacked_kwargs(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> Option<UnpackedKwargs<'db>> {
    ...
    fn negate(&self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> Type<'db> {
        ...
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).negate(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).negate(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).negate(db, env),
            ...
            | Type::UnsafeUnion(_) => IntersectionBuilder::new(db, env)
                .add_negative(*self)
                .build(),
        }
    }
    ...
    fn is_spellable(&self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> bool {
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).is_spellable(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).is_spellable(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).is_spellable(db, env),
            Type::LiteralValue(_)
            | Type::Never
            | Type::NewTypeInstance(_)
            ...
            Type::TypeForm(typeform) => typeform.type_argument(db).is_spellable(db, env),
            ...
            Type::Union(union) => union.elements(db).iter().all(|ty| ty.is_spellable(db, env)),
        }
    }
    ...
    fn is_hintable(&self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> bool {
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).is_hintable(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).is_hintable(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).is_hintable(db, env),
            Type::NominalInstance(_)
            | Type::NewTypeInstance(_)
            | Type::LiteralValue(_)
    ...
    fn promote_collection_element_type(
        ...
        self.promote_collection_element_type_impl(
            db,
            env,
            None,
    ...
    /// As [`Type::promote_collection_element_type`], but honouring the numeric model of `file`.
    ///
    ...
    fn promote_collection_element_type_impl(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        file: Option<ruff_db::files::File>,
        allow_tuple_size_promotion: bool,
        unconstrained: bool,
    ) -> Type<'db> {
        let ty = if unconstrained {
            match file {
                Some(file) => self.promote_in(db, env, file),
                None => self.promote(db, env),
            }
        } else {
            self
        };
    ...
    fn promote_impl_keeping_literals(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> Type<'db> {
    ...
    fn recursive_type_normalized_impl(
        ...
        match self {
            ...
            Type::Restricted(restricted) => restricted
                ...
                .recursive_type_normalized_impl(db, env, div, true)
                .map(|ty| {
                    RestrictedType::from_type_expression(db, env, restricted.modifier(db), ty)
                }),
            Type::Deferred(deferred) => {
                let mut operands = Vec::with_capacity(deferred.operands(db).len());
                for operand in deferred.operands(db) {
                    operands.push(operand.recursive_type_normalized_impl(db, env, div, true)?);
                }
                Some(deferred.re_evaluate(db, env, operands.into_boxed_slice()))
            }
    ...
    fn is_singleton(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> bool {
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).is_singleton(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).is_singleton(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).is_singleton(db, env),
    ...
    fn find_name_in_mro_with_policy(
        ...
        match self {
            ...
            Type::Deferred(deferred) => deferred
                .reduced(db, env)
                .find_name_in_mro_with_policy(db, env, name, policy),
            Type::Union(union) => Some(union.map_with_boundness_and_qualifiers(db, env, |elem| {
    ...
    fn instance_member(
        ...
        match self {
            ...
            Type::Restricted(restricted) => {
                restricted.value_type(db).instance_member(db, env, name)
            }
            Type::Deferred(deferred) => deferred.reduced(db, env).instance_member(db, env, name),
            Type::Union(union) => union.map_with_boundness_and_qualifiers(db, env, |elem| {
        ...
        fn member_lookup_with_policy_impl<'db>(
            ...
            match this {
                ...
                Type::UnsafeUnion(unsafe_union) => unsafe_union
                    .map_with_boundness_and_qualifiers(db, env, |elem| {
                        let receiver = Some(receiver.unwrap_or(this));
                ...
                Type::ProtocolInstance(_) | Type::TypeVar(_)
                    if let Some(member) =
                        protocol_class::symbolic_method_member(db, env, this, name, receiver) =>
                {
    ...
    fn bindings_impl(
        ...
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).bindings(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).bindings(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).bindings(db, env),
            Type::Callable(callable) => {
    ...
    fn fallback_to_getattr(
        ...
        let custom_getattr_result = || {
            ...
            if django_settings::is_settings_instance(db, env, self)
                && let Some(setting) = django_settings::settings_member(db, env, name)
            {
        ...
        // A known instance's meta type is a class literal, so a receiver whose own
        // class is `object` roots the lookup below at `object` itself — where
        // `MRO_NO_OBJECT_FALLBACK` is deliberately ignored, so that `object()` still
        // finds `object.__init__`. That would let `object.__getattribute__` answer
        // every attribute with `Any`. Such a receiver has no custom
        // `__getattribute__` to find in the first place. (Instances of `object` are
        // unaffected: their meta type `type[object]` is normalized to `type`, which
        // does honor the policy.)
    ...
    fn to_instance(
        ...
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).to_instance(db, env),
            Type::Restricted(restricted) => restricted.value_type(db).to_instance(db, env),
            Type::Deferred(deferred) => deferred.reduced(db, env).to_instance(db, env),
            Type::Dynamic(_) | Type::Divergent(_) | Type::Never => {
            ...
            Type::UnsafeUnion(unsafe_union) => unsafe_union.to_instance(db, env),
    ...
    fn in_type_expression_impl(
        ...
        match self {
            Type::Overlapping(overlapping) => overlapping.value_type(db, env).in_type_expression(
                db,
                scope_id,
            ...
            Type::Deferred(deferred) => deferred.reduced(db, env).in_type_expression(
                db,
                scope_id,
                typevar_binding_context,
                inference_flags,
            ...
            Type::ClassLiteral(class) => {
                ...
                // the vendored typeshed is `.byi`, so it lands here with every
                // other basedpython source: the promotion does not apply to it.
                // the stubs say what they mean instead — the `numeric-promotion`
                // typeshed patch writes `int | float` into every parameter that
                // was relying on the spec to supply the `int`, and leaves the
                // positions that only ever *produce* a number alone
                //
                ...
                let is_by = file.source_type(db).is_basedpython()
                    || db.analysis_settings(file).strict_float;
                let ty = match class.known(db) {
                    Some(KnownClass::Complex) if !is_by => KnownUnion::Complex.to_type(db, env),
            ...
            Type::GenericAlias(alias) => {
                ...
                Ok(Type::instance(db, env, ClassType::from(*alias)))
            }
            ...
            Type::KnownInstance(known_instance) => match known_instance {
                ...
                KnownInstanceType::TypeAliasType(alias) => {
                    Ok(match_type_application(db, env, *alias).unwrap_or(Type::TypeAlias(*alias)))
                }
                ...
                KnownInstanceType::WrappedOptionalClass(ty) => Ok(ty.inner(db)),
    ...
    pub fn nominal_class_is_exact(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> bool {
        match self {
    ...
    fn to_meta_type_with_recursion(
        ...
        fn to_meta_type_inner<'db>(
            ...
            match ty {
                ...
                Type::Deferred(deferred) => {
                    to_meta_type_inner(db, env, deferred.reduced(db, env), context, visitor)
                }
                ...
                Type::UnsafeUnion(unsafe_union) => unsafe_union.map_elements(db, env, |element| {
                    to_meta_type_inner(db, env, element, context, visitor)
                }),
                ...
                Type::LiteralValue(literal) => match literal.kind() {
                    ...
                    LiteralValueTypeKind::Enum(enum_literal) => {
                        ...
                        let variant_class =
                            enum_literal.enum_class(db).as_static().and_then(|class| {
                        ...
                        Type::ClassLiteral(
                            variant_class.unwrap_or_else(|| enum_literal.enum_class(db)),
                        )
                    }
                    LiteralValueTypeKind::String(_)
                    | LiteralValueTypeKind::LiteralString
                    ...
                    LiteralValueTypeKind::Complex(_) => {
                        KnownClass::Complex.to_class_literal(db, env)
                    }
                },
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        match self {
            Type::TypeVar(bound_typevar) => {
                bound_typevar.apply_type_mapping_impl(db, env, type_mapping, visitor)
            }
            Type::KnownInstance(known_instance) => {
                known_instance.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
            }

            // Re-evaluate the deferred operation against the mapped operands: once a
            // specialization substitutes the type parameter, `Dim + 1` with `Dim = 5`
            // folds to `Literal[6]` (and stays symbolic while still unresolved).
            Type::Deferred(deferred) => {
                let operands = deferred
                    .operands(db)
                    .iter()
                    .map(|operand| {
                        operand.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                    })
                    .collect();
                ...
                ) =>
            {
                match instance.known_class(db) {
                    ...
                    _ => instance.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                }
            }
                ...
                ) =>
            {
                if instance.is_singleton(db) {
                    ...
                    instance.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                }
            }
            ...
            Type::NewTypeInstance(newtype) => visitor.visit(db, self, type_mapping, || {
                Type::NewTypeInstance(newtype.map_base_class_type(db, |class_type| {
                    class_type.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                }))
            }),
            ...
            Type::KnownBoundMethod(KnownBoundMethodType::PropertyDunderGet(property)) => {
                Type::KnownBoundMethod(KnownBoundMethodType::PropertyDunderGet(
                    property.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                ))
            }
            ...
            Type::SubclassOf(subclass_of) => {
                subclass_of.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
            }
            ...
            Type::UnsafeUnion(unsafe_union) => match type_mapping {
                ...
                TypeMapping::Materialize(MaterializationKind::Bottom) => unsafe_union
                    ...
                    .fold(IntersectionBuilder::new(db, env), |builder, element| {
                        builder.add_positive(element.apply_type_mapping_impl(
                            db,
                            env,
                            type_mapping,
                            tcx,
            ...
            Type::Overlapping(overlapping) => visitor.visit(db, self, type_mapping, || {
                OverlappingType::from_type_expression(
                    db,
                    overlapping.type_argument(db).apply_type_mapping_impl(
                        db,
                        env,
                        type_mapping,
                        tcx,
            ...
            Type::Restricted(restricted) => visitor.visit(db, self, type_mapping, || {
                let inner = restricted.type_argument(db).apply_type_mapping_impl(
                    db,
                    env,
                    type_mapping,
                    tcx,
                ...
                RestrictedType::from_type_expression(db, env, restricted.modifier(db), inner)
            }),
    ...
    fn find_legacy_typevars_impl(
        ...
        match self {
            ...
            Type::Deferred(deferred) => {
                for operand in deferred.operands(db) {
                    operand.find_legacy_typevars_impl(db, env, binding_context, typevars, visitor);
                }
            }
    ...
    fn str(&self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> Type<'db> {
        match self {
            Type::LiteralValue(literal) => match literal.kind() {
                ...
                LiteralValueTypeKind::Float(_) | LiteralValueTypeKind::Complex(_) => {
                    self.repr(db, env)
                }
            },
    ...
    pub fn definition(
        ...
        match self {
            Self::Overlapping(overlapping) => overlapping.value_type(db, env).definition(db, env),
            Self::Restricted(restricted) => restricted.value_type(db).definition(db, env),
            Self::Deferred(deferred) => deferred.reduced(db, env).definition(db, env),
...
impl<'db> VarianceInferable<'db> for Type<'db> {
    fn variance_of(
        ...
        let v = match self {
            ...
            Type::Overlapping(overlapping_type) => overlapping_type.variance_of(db, env, typevar),
            Type::Restricted(restricted_type) => restricted_type.variance_of(db, env, typevar),
...
enum InvalidTypeExpression<'db> {
    ...
    /// `typing.Self` cannot bound a type parameter of the class it belongs to.
    TypingSelfInClassTypeParameterBound,
        ...
        impl std::fmt::Display for Display<'_> {
            fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                ...
                match self.error {
                    ...
                    InvalidTypeExpression::TypingSelfInClassTypeParameterBound => f.write_str(
                        "`Self` cannot bound a type parameter of the class it belongs to",
                    ),
