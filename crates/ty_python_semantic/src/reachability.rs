...
#[salsa::tracked(
    ...
    cycle_fn = |db: &'db dyn Db, cycle, previous: &Type<'db>, result: Type<'db>, predicate: PatternPredicate<'db>, _| {
        let env = ProgramEnvironment::from_scope(predicate.scope(db));
...
impl<'a, 'db> NarrowingProjector<'a, 'db> {
    ...
    fn predicate_constraints(
        ...
        let constraints =
            infer_narrowing_constraints(db, env, self.predicates[predicate_id], self.place);
        self.graph
...
fn analyze_single_pattern_predicate_kind<'db>(
    ...
    match predicate_kind {
        ...
        PatternPredicateKind::And(predicates) => predicates
            .iter()
            .map(|predicate| {
                analyze_single_pattern_predicate_kind(db, env, predicate, subject_ty, None)
            })
