...
pub(crate) fn register_lints(registry: &mut LintRegistryBuilder) {
    ...
    registry.register_lint(&FINAL_ON_VARIABLE);
...
declare_lint! {
    ...
    /// let a = 1
    /// ```
    pub(crate) static FINAL_ON_VARIABLE = {
...
declare_lint! {
    ...
    pub(crate) static SHADOWED_RECEIVER_MEMBER = {
        summary: "detects a block declaration that shadows the receiver's member of the same name",
        status: LintStatus::stable("0.0.71"),
        default_level: Level::Warn,
    }
}
...
pub(super) fn report_invalid_assignment<'db>(
    ...
    {
        let model = crate::SemanticModel::new(context.db(), db.program_file(context.file()));
...
pub(super) fn report_invalid_reified_override<'db>(
        ...
        ReifiedOverrideError::Bound {
            ...
            diagnostic.info(format_args!(
                "the bound of type parameter `{sub_name}` (`{}`) rejects specializations \
                 ...
                 contravariant",
                sub_admissible.display(db, env),
                base_admissible.display(db, env),
            ));
        }
    }
    ...
    if let Place::Defined(DefinedPlace {
        ...
        .class_member(db, env, member, MemberLookupPolicy::default())
        .place
    {
        diagnostic.annotate(
    ...
    match superclass_method_kind {
        ...
        MethodKind::Synthesized(class_kind) => {
            ...
            let mut sub = match class_kind {
                ...
                CodeGeneratorKind::SqlalchemyDeclarative => make_sub(format_args!(
                    "`{overridden_method}` is a generated method created because \
                        `{superclass_name}` is a SQLAlchemy declarative model"
                )),
...
pub(super) fn report_incompatible_base_method<'db>(
    ...
    let env = context.program_environment();
    let class_name = ClassLiteral::Static(class).display_with(db, env, settings.clone());
