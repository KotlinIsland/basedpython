...
use super::protocol_class::{InlineProtocolMember, ProtocolInterface, ProtocolInterfaceView};
use super::{
...
impl<'db> Type<'db> {
        ...
        M: IntoIterator<Item = (&'a str, Type<'db>)>,
    {
        Self::ProtocolInstance(ProtocolInstanceType::synthesized(
            SynthesizedProtocolType::stated(
                db,
                ProtocolInterface::with_property_members(db, env, members),
            ),
        ))
    }
    ...
    {
        Self::ProtocolInstance(ProtocolInstanceType::synthesized(
            SynthesizedProtocolType::stated(db, ProtocolInterface::with_methods(db, env, methods)),
        ))
    }
    ...
    pub(super) fn mentions_recovered_protocol(
        self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> bool {
        super::visitor::any_over_type(db, env, self, false, |ty| {
            matches!(
                ty,
    ...
    pub(super) fn inline_protocol<M>(
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        members: M,
        packs: Box<[Type<'db>]>,
        ...
        M: IntoIterator<Item = (Name, InlineProtocolMember<'db>)>,
    {
        Self::ProtocolInstance(ProtocolInstanceType::synthesized(
            SynthesizedProtocolType::stated(
                db,
                ProtocolInterface::with_inline_members(db, env, members, packs),
            ),
        ))
    }
    ...
    pub(super) fn recovered_protocol<M>(
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        members: M,
    ) -> Self
        ...
        M: IntoIterator<Item = (Name, InlineProtocolMember<'db>)>,
    {
        Self::ProtocolInstance(ProtocolInstanceType::synthesized(
            SynthesizedProtocolType::recovered(
                db,
                ProtocolInterface::with_inline_members(db, env, members, Box::default()),
            ),
        ))
    }
}
...
impl<'db> NominalInstanceType<'db> {
    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        match self.0 {
            NominalInstanceInner::ExactTuple(tuple) => {
                Type::tuple(tuple.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor))
            }
            ...
            NominalInstanceInner::NonTuple(class) => {
                ...
                let transformed =
                    mapped_class.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor);
    ...
    pub(super) fn find_legacy_typevars_impl(
        ...
        match self.0 {
            ...
            NominalInstanceInner::Regex(regex) => {
                regex.class(db).find_legacy_typevars_impl(
                    db,
                    env,
                    binding_context,
                    typevars,
...
impl<'c, 'db> TypeRelationChecker<'_, 'c, 'db> {
    ...
    pub(super) fn check_nominal_instance_pair(
        ...
        match (source.0, target.0) {
            ...
            _ => {
                if let Some(result) = self.check_anon_named_tuple_pair(
                    db,
                    env,
                    source.class(db, env),
                    target.class(db, env),
    ...
    fn check_anon_named_tuple_pair(
        ...
        for (i, (sf, tf)) in source_fields.iter().zip(target_fields.iter()).enumerate() {
            ...
            if result.is_never_satisfied(db, env) {
                return Some(result);
            }
        }
        Some(result)
    ...
    if visitor.should_visit_lazy_type_attributes() {
        ...
        match protocol.inner {
            ...
            Protocol::Synthesized(synthesized) => {
                walk_protocol_interface(
                    db,
                    ProtocolInterfaceView::new(synthesized.interface(db), None),
                    visitor,
                );
            }
        }
...
impl<'db> ProtocolInstanceType<'db> {
    ...
    pub(crate) fn instance_member(
        ...
        match self.inner {
            ...
            Protocol::Synthesized(synthesized) => {
                synthesized.interface(db).instance_member(db, env, name)
            }
    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        match self.inner {
            ...
            Protocol::Synthesized(synthesized) => Self::synthesized(
                synthesized.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
            ),
...
mod synthesized_protocol {
    ...
    impl<'db> SynthesizedProtocolType<'db> {
        ...
        pub(super) fn find_legacy_typevars_impl(
            ...
            self.interface(db).find_legacy_typevars_impl(
                db,
                env,
                binding_context,
                typevars,
    ...
    impl<'db> VarianceInferable<'db> for SynthesizedProtocolType<'db> {
        fn variance_of(
            ...
            self.interface(db).variance_of(db, env, typevar)
        }
    }
}
