    ...
    pub(crate) fn apply_type_mapping_impl(
        ...
        for (constraint_id, constraint) in constraints {
            ...
            let lower = constraint
                .bounds
                .lower
                .map(|lower| lower.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor));
            let upper = constraint
                .bounds
                .upper
                .map(|upper| upper.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor));
            ...
            TypeVarBoundOrConstraints::UpperBound(bound) => {
                ...
                let path_lower = match (path_bound.lower, bound_typevar.typevar(db).lower_bound(db))
                {
                    (Some(inferred), Some(declared)) => {
                        Some(UnionType::from_two_elements(db, env, inferred, declared))
                    }
        ...
        match self.typevar(db).bound_or_constraints(db, env) {
            ...
            Some(TypeVarBoundOrConstraints::UpperBound(bound)) => {
                ...
                let lower = self
                    .typevar(db)
                    .lower_bound(db)
                    .map(|lower| lower.bottom_materialization(db, env));
                Constraint::new_node_with_bounds(db, env, storage, self, lower, Some(bound))
            }
        ...
        match self.typevar(db).bound_or_constraints(db, env) {
            ...
            Some(TypeVarBoundOrConstraints::UpperBound(bound)) => {
                ...
                let lower = self
                    .typevar(db)
                    .lower_bound(db)
                    .map(|lower| lower.top_materialization(db, env));
                (
                    Constraint::new_node_with_bounds(db, env, storage, self, lower, Some(bound)),
                    Vec::new(),
                )
