    ...
    pub(crate) fn apply_type_mapping_impl(
        ...
        for (constraint_id, constraint) in constraints {
            ...
            let lower = constraint.stored_lower_bound().map(|lower| {
                lower.map(|ty| ty.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor))
            });
            let upper = constraint.stored_upper_bound().map(|upper| {
                upper.map(|ty| ty.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor))
            });
    ...
    pub(crate) fn preliminary_solve(
            ...
            .require_bound_or_constraints(db, env)
        {
            TypeVarBoundOrConstraints::UpperBound(bound) => {
                ...
                if path_bound.evidence_lower.is_some() || declared_lower.is_some() {
