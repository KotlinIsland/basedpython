...
use crate::types::{
    ...
    RestrictedType, StaticClassLiteral, SubclassOfType, Type, TypeAliasType, TypeFormType,
    TypeGuardType, TypeIsType, TypedDictType, UnionType, UnsafeUnionType,
...
pub(crate) trait TypeVisitor<'db> {
    ...
    fn visit_restricted_type(&self, db: &'db dyn Db, restricted: RestrictedType<'db>) {
        walk_restricted_type(db, restricted, self);
    }
    ...
    fn from(ty: Type<'db>) -> Self {
        match ty {
            ...
            Type::Restricted(restricted) => {
                TypeKind::NonAtomic(NonAtomicType::Restricted(restricted))
            }
...
pub(super) fn walk_non_atomic_type<'db, V: TypeVisitor<'db> + ?Sized>(
    ...
    match non_atomic_type {
        ...
        NonAtomicType::Restricted(restricted) => {
            visitor.visit_restricted_type(db, restricted);
        }
