...
fn structural_interface<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    ty: Type<'db>,
) -> Option<ProtocolInterface<'db>> {
    match ty {
        ...
        Type::TypeVar(bound_typevar) => {
            structural_interface(db, env, bound_typevar.typevar(db).upper_bound(db, env)?)
        }
        _ => None,
    }
}
...
pub(super) fn walk_protocol_interface<'db, V: super::visitor::TypeVisitor<'db> + ?Sized>(
    ...
    for pack in interface.base().pending_packs(db) {
...
impl<'db> ProtocolInterface<'db> {
        ...
        M: IntoIterator<Item = (&'a str, Type<'db>)>,
    {
        let members: BTreeMap<_, _> = members
            ...
            .collect();
        Self::new(db, env.program(db), members, Box::default())
    }
        ...
        M: IntoIterator<Item = (Name, InlineProtocolMember<'db>)>,
    {
        let members: BTreeMap<_, _> = members
            .into_iter()
            .map(|(name, member)| (name, ProtocolMemberData::from_inline(db, env, member)))
            .collect();
        Self::new(db, env.program(db), members, packs)
    }

    fn empty(db: &'db dyn Db, env: &ProgramEnvironment<'db>) -> Self {
        Self::new(db, env.program(db), BTreeMap::default(), Box::default())
    }
    ...
    fn cycle_normalized(
        ...
        let members: BTreeMap<_, _> = curr_inner
            ...
            .collect();
        Self::new(db, env.program(db), members, self.pending_packs(db).clone())
    }
    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        let mut members: BTreeMap<_, _> = self
            ...
            .map(|(name, data)| {
                (
                    name.clone(),
                    data.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                )
            })
            .collect();
        ...
        for pack in packs {
            let pack = pack.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor);
        ...
        Self::new(db, env.program(db), members, pending.into_boxed_slice())
    }
    ...
    pub(super) fn find_legacy_typevars_impl(
        ...
        for pack in self.pending_packs(db) {
            pack.find_legacy_typevars_impl(db, env, binding_context, typevars, visitor);
        }
    }
...
impl<'db> ProtocolMemberWrite<'db> {
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        match self {
            Self::Type(member) => {
                Self::Type(member.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor))
            }
            Self::Descriptor { descriptor, domain } => Self::Descriptor {
                descriptor: descriptor.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                domain: domain.map(|domain| {
                    domain.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                }),
            },
        }
    }
...
impl<'db> ProtocolMemberData<'db> {
    ...
    fn from_inline(
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        member: InlineProtocolMember<'db>,
    ) -> Self {
        match member {
            InlineProtocolMember::Attribute(ty) => {
                Self::attribute(ty, TypeQualifiers::default(), None)
            }
            InlineProtocolMember::Method(callable) => Self::method(db, env, callable, None),
...
impl<'db> ProtocolMemberKind<'db> {
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        match self {
            Self::Method(member, kind) => Self::Method(
                member.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                kind,
            ),
            Self::Property { read, write } => Self::Property {
                read: read
                    .map(|read| read.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)),
                write: write.map(|write| {
                    write.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                }),
            },
...
impl<'a, 'db> ProtocolMember<'a, 'db> {
    ...
    pub(super) fn reified_member_shape(
        &self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> Option<ReifiedMember<'db>> {
        if self.is_method() {
        ...
        let instance = self.capabilities(db, env).instance;
        let read = instance
            .read
            .and_then(|member| member.resolve(db, env))
            .map(ProtocolMemberType::ty);
...
impl<'c, 'db> TypeRelationChecker<'_, 'c, 'db> {
    ...
    fn check_property_write(
        ...
        self.check_property_write_requirement(db, env, &requirement, member_name, value_ty)
    }
...
fn cached_protocol_interface<'db>(
    ...
    ProtocolInterface::new(db, env.program(db), members, Box::default())
}
