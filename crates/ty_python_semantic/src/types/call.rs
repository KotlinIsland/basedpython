...
impl<'db> Type<'db> {
    ...
    pub(crate) fn try_call_bin_op_return_type_with_tcx(
        ...
        #[salsa::tracked(returns(copy), cycle_initial=|_, _, _, _, _, _, _| None, heap_size=ruff_memory_usage::heap_size)]
        ...
        try_call_bin_op_return_type_impl(db, env.program(db), left_ty, op, right_ty, tcx)
    }
