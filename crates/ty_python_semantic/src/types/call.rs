...
impl<'db> Type<'db> {
    ...
    /// type of the whole expression) into the dunder call, so that a `Never`-defaulted output
    /// parameter can widen to the assignment target — e.g. `x: list[int | None] = a + a`.
    pub(crate) fn try_call_bin_op_result_with_tcx(
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
        left_ty: Type<'db>,
        op: ast::Operator,
