...
use crate::types::typed_dict::{
    SynthesizedTypedDictType, TypedDictField, TypedDictFieldBuilder, TypedDictOpenness,
    TypedDictSchema, deferred_functional_typed_dict_openness,
    ...
    fn recursive_type_normalized_impl(
        ...
        match self {
            ...
            Self::Synthesized {
                ...
                packs: packs
                    .iter()
                    .map(|pack| pack.recursive_type_normalized_impl(db, env, div, true))
...
impl<'db> DynamicTypedDictLiteral<'db> {
    ...
    pub(crate) fn openness(self, db: &'db dyn Db) -> TypedDictOpenness<'db> {
        match self.anchor(db) {
            ...
            // basedpython synthesized `{"key": T}` type literals behave like a
            // default (implicitly open) `TypedDict`
            DynamicTypedDictAnchor::Synthesized { .. } => TypedDictOpenness::ImplicitlyOpen,
        }
    }
