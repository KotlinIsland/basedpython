...
bitflags::bitflags! {
    /// Boolean facts about a class definition header.
    #[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
    pub struct ClassLiteralFlags: u8 {
        ...
        const HAS_DECORATORS = 1 << 1;
        /// Whether this class has PEP 695 type parameters.
        const HAS_TYPE_PARAMS = 1 << 2;
...
impl get_size2::GetSize for ClassLiteralFlags {}
...
impl<'db> StaticClassLiteral<'db> {
    ...
    pub(crate) fn valueless_declarations(self, db: &'db dyn Db) -> Box<[Name]> {
        let module = parsed_module(db, self.program_file(db).python_file(db)).load(db);
        self.node(db, &module)
            .body
            .iter()
            .filter_map(|stmt| match stmt {
                ast::Stmt::AnnAssign(annotated) if annotated.value.is_none() => {
                    Some(annotated.target.as_name_expr()?.id.clone())
                }
                _ => None,
            })
            .collect()
    ...
    pub(crate) fn explicit_bases(self, db: &'db dyn Db) -> &'db [Type<'db>] {
        ...
        fn explicit_bases_inner<'db>(
            ...
            if class_stmt.has_synthetic_marker("enum_class") {
                bases.push(KnownClass::Enum.to_class_literal(db, env));
            }
            ...
            if class_stmt.is_enum_variant() {
                ...
                if let Some(enum_literal) = index
                    .ancestor_scopes(class.body_scope(db).file_scope_id(db))
                    .skip(1)
                    .find_map(|(_, ancestor_scope)| {
                        let class = ancestor_scope.node().as_class()?;
                        let definition = index.expect_single_definition(class);
        ...
        // basedpython classes can gain injected bases (synthetic enum/protocol
        // markers, enum variants) even with no explicit bases in the header, so
        // the fast path only applies to standard python files
        let is_by = matches!(self.file(db).path(db).extension(), Some("by" | "byi"));
        if !is_by && !self.has_explicit_bases(db) {
            return &[];
        }
    ...
    pub(crate) fn sealed_members(self, db: &'db dyn Db) -> Box<[StaticClassLiteral<'db>]> {
        let global = ty_python_core::global_scope(db, self.program_file(db));
    ...
    pub(crate) fn own_synthesized_member(
        ...
        if name == "__sealed_members__" && self.is_sealed(db) {
            let elements = self.sealed_members(db).iter().map(|member| {
                SubclassOfType::from(
                    db,
                    env,
                    ClassLiteral::Static(*member).default_specialization(db),
                )
            });
            return Some(Type::heterogeneous_tuple(db, env, elements));
        }
        ...
        match (field_policy, name) {
            ...
            (field_policy @ CodeGeneratorKind::DataclassLike(_), "__match_args__")
                if env.python_version(db) >= PythonVersion::PY310 || self.is_enum_variant(db) =>
            {
    ...
    fn own_fields_inner(
        ...
        if field_policy == CodeGeneratorKind::Django {
            return OwnClassFields {
                fields: self.django_own_fields(db, env, specialization),
                class_variables: Box::default(),
            };
        }

        if field_policy == CodeGeneratorKind::SqlalchemyDeclarative {
            return OwnClassFields {
                fields: self.sqlalchemy_own_fields(db, env, specialization),
                class_variables: Box::default(),
            };
        }
    ...
    fn django_own_fields(
        ...
        for (symbol_id, bindings) in use_def.all_end_of_scope_symbol_bindings() {
            ...
            let Some(ty) = place_from_bindings(db, env, bindings)
                .place
                .ignore_possibly_undefined()
            else {
                continue;
            };
            if !django::is_field_instance(db, env, ty) {
                continue;
            }
        ...
        for (_, symbol_id, ty, definition) in field_bindings {
            ...
            let field = Field {
                ...
                kind: FieldKind::Django {
                    ...
                    null: facts.null,
                    many_to_many: django::is_many_to_many_instance(db, env, ty),
    ...
    fn sqlalchemy_own_fields(
        ...
        for (_, symbol_id, result) in field_declarations {
            ...
            let Some(field_ty) = sqlalchemy::mapped_field_type(db, env, attr_ty) else {
                continue;
            };
            fields.insert(
                symbol.name().clone(),
...
impl<'db> StaticClassLiteral<'db> {
    /// basedpython: whether no member of this class mentions `typevar`.
    ///
    /// The typing spec reports an inferred-bivariant class parameter as covariant, and does so
    /// because the parameter is *unused* — nothing can tell two specializations apart. This fork
    /// also concludes bivariance for a parameter that only a private member mentions, which is a
    /// parameter the class genuinely uses, so that conclusion has to survive the spec's fallback.
    ///
    /// Re-reading the class with `bivariant-private-attributes` switched off separates the two:
    /// with the fork's rule disabled a private member is merely immutable, and so reports
    /// covariance, leaving bivariance to mean what the spec means by it.
    pub(crate) fn typevar_is_unused(
        self,
        db: &'db dyn Db,
        typevar: BoundTypeVarIdentity<'db>,
    ) -> bool {
...
impl<'db> StaticClassLiteral<'db> {
    ...
    /// use their structural interface; `TypedDict` classes use their fields. Other classes retain
    /// the ordinary attribute and base-class variance rules.
...
impl<'db> StaticClassLiteral<'db> {
    ...
    pub(crate) fn own_variance_of_with(
        ...
        VarianceTerm::join(
            db,
            attribute_variances.chain(unfrozen_inherited_field_variances),
        )
    }
}
...
pub(crate) fn based_enum_variant_union<'db>(
    ...
    for stmt in &class_stmt.body {
            ...
            && variant.is_enum_variant()
        {
            if variant.has_synthetic_marker("variant_unit") {
                ...
                if let Some(enum_class) = ClassLiteral::Static(class).into_enum_class(db) {
                    ...
                    .and_then(ClassLiteral::as_static)
                {
                    ...
                    binding_type(db, definition).to_instance_approximation(db, env)
                {
    ...
    (!elements.is_empty()).then(|| UnionType::from_elements(db, env, elements))
}
