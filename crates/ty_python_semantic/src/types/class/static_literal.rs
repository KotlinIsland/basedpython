...
use crate::{
    ...
    types::{
        ...
        TypeMapping, TypeVarVariance, TypedDictModule, UnionBuilder, UnionType, binding_type,
            ...
            inner: if is_attribute_bound {
                Place::bound(
                    union_of_inferred_types
                        .build()
                        .promote_in(db, env, class_body_scope.file(db))
...
impl<'db> VarianceInferable<'db> for StaticClassLiteral<'db> {
    fn variance_of(
        ...
        self.variance_of_owner(db, typevar, bivariant_private_attributes)
    }
}
...
impl<'db> StaticClassLiteral<'db> {
    ...
    pub(crate) fn typevar_is_unused(
        ...
        self.variance_of_owner(db, typevar, false) == TypeVarVariance::Bivariant
    }
