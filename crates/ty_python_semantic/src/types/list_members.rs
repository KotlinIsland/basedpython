    ...
    fn extend_with_type(&mut self, db: &'db dyn Db, env: &ProgramEnvironment<'db>, ty: Type<'db>) {
        match ty {
            ...
            Type::Overlapping(overlapping) => {
                self.extend_with_type(db, env, overlapping.value_type(db, env));
            }
            Type::Restricted(restricted) => {
                self.extend_with_type(db, env, restricted.value_type(db));
            }
            Type::Deferred(deferred) => {
                self.extend_with_type(db, env, deferred.reduced(db, env));
            }
            Type::Union(union) => {
        ...
        match CodeGeneratorKind::from_class(db, class_literal) {
            ...
            Some(field_policy @ CodeGeneratorKind::Django) => {
                ...
                if let Some(class) = class_literal.as_static() {
                    ...
                    for name in crate::types::dedicated::django::synthesized_member_names(
                        db, env, class, fields,
                    ) {
                        if let Place::Defined(DefinedPlace {
                            ty: synthetic_member,
                            ..
                        }) = ty.member(db, env, name.as_str()).place
                        {
                            self.members.insert(Member {
                                name,
                                ty: synthetic_member,
