...
pub(crate) use self::static_literal::{
    ClassLiteralFlags, ExpandedClassBaseEntry, FrozenDataclassDispatch, StaticClassLiteral,
    based_enum_has_payload_variants, based_enum_of_variant, based_enum_unit_member_names,
    based_enum_unit_variant_class, based_enum_variant_union, expanded_class_base_entries,
};
...
impl<'db> GenericAlias<'db> {
    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        let tcx = tcx
            .annotation()
            .and_then(|ty| ty.specialization_of(db, env, self.origin(db)))
            .map(|specialization| specialization.types(db))
            .unwrap_or(&[]);

        let original_specialization = self.specialization(db);
        let specialization =
            original_specialization.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor);
...
impl<'db> ClassType<'db> {
        ...
        match self {
            ...
            Self::NonGeneric(ClassLiteral::DynamicTypedDict(typeddict)) => {
                Self::NonGeneric(ClassLiteral::DynamicTypedDict(
                    typeddict.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor),
                ))
            }
            Self::NonGeneric(_) => self,
            Self::Generic(generic) => {
                Self::Generic(generic.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor))
            }
        }
    }
    ...
    pub(super) fn could_exist_in_mro_of_with_disjointness_checker<'c>(
        ...
        self.could_exist_in_mro_of_impl(db, other, |this, other| {
            checker
                .check_specialization_pair(db, env, this, other)
                .is_always_satisfied(db, env)
        })
    }
