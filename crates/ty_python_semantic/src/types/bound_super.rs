...
use crate::{
    ...
    types::{
        ...
        TypeVarBoundOrConstraints, UnionBuilder, UnsafeUnionType,
...
impl<'db> BoundSuperType<'db> {
    ...
    pub(crate) fn build(
        ...
        if let Some(gradual) =
            crate::types::inferred_signature::gradual_hole(db, env, pivot_class_type)
        {
            return BoundSuperType::build(db, env, gradual, owner_type);
        }
        if let Some(gradual) = crate::types::inferred_signature::gradual_hole(db, env, owner_type) {
            return BoundSuperType::build(db, env, pivot_class_type, gradual);
        }
        ...
        let owner = match owner_type {
            ...
            Type::UnsafeUnion(unsafe_union) => {
                ...
                return Ok(UnsafeUnionType::from_inferred_elements(db, env, elements));
            }
            ...
            Type::Overlapping(overlapping) => {
                return delegate_to(overlapping.value_type(db, env));
            }
            Type::Restricted(restricted) => {
                return delegate_to(restricted.value_type(db));
            }
            Type::Deferred(deferred) => {
                return delegate_to(deferred.reduced(db, env));
            }
