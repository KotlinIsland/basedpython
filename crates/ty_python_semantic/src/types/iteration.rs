...
pub(crate) fn report_iteration_over_character<'db>(
    ...
    if !instance.class(db, env).is_known(db, KnownClass::Character) {
        return;
    }
    ...
    pub(super) fn try_iterate_with_mode(
        ...
        fn non_async_special_case<'db>(
            ...
            match ty {
                ...
                Type::NewTypeInstance(newtype) => non_async_special_case(db, env, newtype.concrete_base_type(db)),
                Type::Overlapping(overlapping) => non_async_special_case(db, env, overlapping.value_type(db, env)),
                Type::Restricted(restricted) => non_async_special_case(db, env, restricted.value_type(db)),
                Type::Deferred(deferred) => non_async_special_case(db, env, deferred.reduced(db, env)),
