...
fn asserts_guard_targets<'db>(
    ...
    let module = parsed_module(db, call_expr.program_file(db).python_file(db)).load(db);
...
#[salsa::tracked(
    ...
    cycle_fn=|db: &'db dyn Db, cycle, previous: &PatternSuccessTypes<'db>, result: PatternSuccessTypes<'db>, pattern: PatternPredicate<'db>| {
        let env = ProgramEnvironment::from_scope(pattern.scope(db));
...
pub(crate) fn pattern_success_types<'db>(
    ...
    let env = ProgramEnvironment::from_scope(pattern.scope(db));
    let incoming_subject_ty = pattern_subject_type(db, subject);
...
impl ClassInfoConstraintFunction {
    ...
    fn generate_constraint<'db>(
        ...
        match classinfo {
            ...
            Type::Overlapping(overlapping) => self.generate_constraint(
                db,
                env,
            ...
            Type::Restricted(restricted) => self.generate_constraint(
                db,
                env,
            ...
            Type::Deferred(deferred) => self.generate_constraint(
                db,
                env,
            ...
            Type::UnsafeUnion(unsafe_union) => self.generate_constraint(
                db,
                env,
                unsafe_union.to_union(db, env),
                is_positive,
                use_generic_filtering,
            ),
...
fn necessary_match_pattern_type<'db>(
    ...
    match pattern {
        ...
        PatternPredicateKind::And(predicates) => predicates
            .iter()
            .fold(IntersectionBuilder::new(db, env), |builder, predicate| {
                builder.add_positive(necessary_match_pattern_type(db, env, predicate))
            })
            .build(),
...
impl<'db> NarrowingConstraintsBuilder<'db, '_> {
    ...
    fn evaluate_expr_compare(
        ...
        if matches!(&**ops, [ast::CmpOp::In | ast::CmpOp::NotIn])
            ...
            && is_or_contains_typeddict(db, &env, rhs_type)
        {
                ...
                && let Some(narrowable) = PlaceExpr::try_from_expr(left)
            {
                ...
                if let Some(constraint_ty) = constraint {
                    ...
                    let constraint = NarrowingConstraint::intersection(
                        constraint_ty.negate_if(self.db, &env, !positive),
                    );
                    constraints
                        .entry(place)
                        .and_modify(|existing| {
    ...
    fn evaluate_expr_call(
        ...
        // type wholesale: the cast is an assertion that overrides the static
        // type, so the whole prior type is replaced (unlike `isinstance`, which
        // intersects). `cast!` asserts it at runtime too; the plain `cast` is
        // only accepted when the value already is the target. The safe `cast?`
        // form can yield `None`, so it is left to ordinary assignment narrowing
        // and does not narrow its operand. There is no negative branch for an
        // unconditional cast statement.
        if matches!(
            expr_call.cast_kind,
    ...
    fn narrow_typeddict_subscript(
        ...
        if !is_or_contains_typeddict(db, &env, subscript_value_type) {
            return None;
        }
    ...
    fn narrow_with_present_key(&self, ty: Type<'db>, key: &str) -> Type<'db> {
        ...
        match ty.resolve_type_alias(db) {
            ...
            resolved if is_or_contains_typeddict(db, &env, resolved) => constrain(
                ty,
...
fn is_or_contains_typeddict<'db>(
    ...
    match ty {
        Type::Overlapping(overlapping) => {
            is_or_contains_typeddict(db, env, overlapping.value_type(db, env))
        }
        Type::Restricted(restricted) => {
            is_or_contains_typeddict(db, env, restricted.value_type(db))
        }
        Type::Deferred(deferred) => is_or_contains_typeddict(db, env, deferred.reduced(db, env)),
        ...
        Type::Intersection(intersection) => {
            intersection
                ...
                .any(|intersection_element_ty| {
                    is_or_contains_typeddict(db, env, *intersection_element_ty)
                })
        }
        Type::Union(union) => union
            .elements(db)
            .iter()
            .any(|union_member_ty| is_or_contains_typeddict(db, env, *union_member_ty)),
        Type::UnsafeUnion(unsafe_union) => unsafe_union
            .elements(db)
            .iter()
            .any(|element| is_or_contains_typeddict(db, env, *element)),
        Type::TypeAlias(alias) => is_or_contains_typeddict(db, env, alias.value_type(db)),
...
fn all_matching_typeddict_fields_have_literal_types<'db>(
    ...
    match ty {
        ...
        Type::Union(union) => union.elements(db).iter().all(|union_member_ty| {
            !is_or_contains_typeddict(db, env, *union_member_ty)
                || all_matching_typeddict_fields_have_literal_types(
                    db,
                    env,
                    *union_member_ty,
        ...
        Type::UnsafeUnion(unsafe_union) => unsafe_union.elements(db).iter().all(|element| {
            !is_or_contains_typeddict(db, env, *element)
                || all_matching_typeddict_fields_have_literal_types(db, env, *element, field_name)
        }),
        ...
        Type::Deferred(deferred) => all_matching_typeddict_fields_have_literal_types(
            db,
            env,
            deferred.reduced(db, env),
            field_name,
        ...
        Type::Intersection(intersection) => {
            intersection
                ...
                .all(|intersection_member_ty| {
                    !is_or_contains_typeddict(db, env, *intersection_member_ty)
                        || all_matching_typeddict_fields_have_literal_types(
                            db,
                            env,
