...
fn spell_class<'db>(db: &'db dyn Db, file: File, class: ClassType<'db>) -> Option<String> {
    match class {
        ...
        ClassType::Generic(alias) => {
            ...
            // tuples carry their precise element shape out-of-band; spell it
            // rather than the class's single typevar
            if origin.is_known(db, KnownClass::Tuple) {
                let spelled = match specialization.tuple(db)? {
                    Tuple::Fixed(fixed) => {
                        let elements = fixed
                            .elements_slice()
                            .iter()
                    ...
                    Tuple::Variable(_) => return None,
                };
                return Some(format!("{base}[{spelled}]"));
            }
