            ...
            match paramspec_value {
                Type::TypeVar(typevar) if typevar.is_parameter_pack(db) => {
                    let prefix_parameters = prefix_parameters
                        .iter()
                        .map(|param| {
                            param.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                        })
                        .collect::<Vec<_>>();
...
impl<'db> Signature<'db> {
    ...
    pub(super) fn apply_type_mapping_impl<'a>(
        ...
        Self {
            ...
            definition: self.definition,
            source_overload_index: self.source_overload_index,
            receiver_constraints: self.map_receiver_constraints(
                db,
                env,
                type_mapping,
                tcx,
    ...
    pub(crate) fn inherit_unannotated_from_overloads(
        ...
        for impl_param in &mut Arc::make_mut(&mut self.parameters.data).value {
            ...
            if !tys.is_empty() {
                impl_param.annotated_type = UnionType::from_elements(db, env, tys);
                impl_param.inferred_annotation = false;
            }
        }
        if inherit_return && !overload_signatures.is_empty() {
            self.return_ty =
                UnionType::from_elements(db, env, overload_signatures.iter().map(|s| s.return_ty));
        }
    }
    ...
    pub(crate) fn inherit_defaults_from(
        ...
        for parameter in &mut Arc::make_mut(&mut self.parameters.data).value {
            ...
            let Some(inherited) = base
                ...
                .and_then(|candidate| candidate.default_type(db))
            else {
                continue;
            };
    ...
    pub(crate) fn open_unannotated_parameter_holes(
        ...
        self.generic_context = Some(match self.generic_context {
            ...
            None => GenericContext::from_typevar_instances(db, env, holes),
        });
    }
    ...
    fn map_constraints(
        ...
        if !constraints
            .types()
            .any(|ty| ty.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor) != ty)
        {
    ...
    fn needs_self_mapping(
        ...
        self.return_ty.contains_self(db, env)
            ...
            || self.generic_context.is_some_and(|generic_context| {
                generic_context
                    .variables(db)
                    .any(|typevar| typevar.bounds_mention_self(db, env))
            })
    }
...
impl<'db> Parameters<'db> {
    ...
    pub(crate) fn from_annotation(
        ...
        for (index, parameter) in parameters.iter().enumerate() {
            ...
            if parameter
                .annotated_type()
                .is_typed_dict_bounded_typevar(db, env)
                && Self::typevar_used_by_another_parameter(db, env, &parameters, index)
            {
            ...
            if let Some(unpacked_typed_dict) = parameter.unpacked_typed_dict(db) {
                ...
                Self::push_unpacked_protocol(db, env, &mut value, unpacked_protocol);
            } else {
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        let value: Box<[_]> = self
            ...
            .value
            .iter()
            .map(|param| param.apply_type_mapping_impl(db, env, &type_mapping, tcx, visitor))
            .collect();
...
impl<'db> Parameter<'db> {
    ...
    pub(crate) fn with_unpacked_kwargs(
        mut self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> Self {
        match self.annotated_type.unpacked_kwargs(db, env) {
    ...
    fn from_node_and_kind(
        ...
        let is_unpacked_typed_dict_kwargs = matches!(&kind, ParameterKind::KeywordVariadic { .. })
            ...
            .is_some()
                ...
                || (annotation_flags.contains(TypeExpressionFlags::UNPACK)
                    && annotated_type.is_typed_dict_bounded_typevar(db, env)));
    ...
    fn apply_type_mapping_impl<'a>(
        ...
        let apply_to_default_type = |default_type: &Option<ParameterDefault<'db>>| {
            default_type.map(|default| match type_mapping {
                ...
                _ => default
                    .map_type(|ty| ty.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)),
            })
        };
