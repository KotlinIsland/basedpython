...
use crate::types::typevar::{
    BoundTypeVarIdentity, PackBoundViolation, TypeVarIdentity, TypeVarInstance, TypeVarSet,
    pack_bound_violation,
};
...
use crate::types::{
    ...
    KnownInstanceType, MaterializationKind, PromotionKind, PromotionMode, SubclassOfInner, Type,
    ...
    for (ancestor_scope_id, ancestor_scope) in index.ancestor_scopes(containing_scope) {
        ...
        if is_class_scope
            && !ancestor_scope.node().as_class().is_some_and(|class_ref| {
                let definition = index.expect_single_definition(class_ref);
                let module =
                    parsed_module(db, definition.program_file(db).python_file(db)).load(db);
        ...
        .and_then(|static_class| {
            if static_class.is_extension(db) {
                crate::types::extensions::body_view_class(db, static_class)
                    .map(|body_view| Type::instance(db, &env, body_view))
            } else {
...
impl<'db> Specialization<'db> {
    ...
    pub(super) fn merge_cycle_recovery(self, db: &'db dyn Db, previous: Self) -> Option<Self> {
        if self.generic_context(db) != previous.generic_context(db)
            ...
            || self.projections(db) != previous.projections(db)
        {
            return None;
        }
    ...
    pub(crate) fn apply_type_mapping_impl<'a>(
        ...
        if let TypeMapping::Materialize(materialization_kind) = type_mapping {
            return self.materialize_impl(db, env, *materialization_kind, visitor);
        }
        ...
        let types = self.map_types(db, |i, typevar, ty| {
            ...
            match (typevar.variance(db), type_mapping) {
                ...
                (variance, _) if variance.is_covariant() => {
                    ty.apply_type_mapping_impl(db, env, type_mapping, tcx, visitor)
                }
                _ => ty.apply_type_mapping_impl(db, env, &type_mapping.flip(), tcx, visitor),
            }
        });

        let original_tuple_inner = self.tuple_inner(db);
        let tuple_inner = original_tuple_inner.map(|tuple| {
            tuple.apply_type_mapping_impl(db, env, type_mapping, TypeContext::default(), visitor)
        });
    ...
    pub(super) fn materialize_impl(
        ...
        let types = self.map_types(db, |_, bound_typevar, vartype| {
            ...
            let top_materialization =
                vartype.materialize(db, env, MaterializationKind::Top, visitor);
            ...
            match variance {
                ...
                TypeVarVariance::Covariant | TypeVarVariance::Contravariant => {
                    ...
                    let materialized =
                        vartype.materialize(db, env, effective_materialization_kind, visitor);
    ...
    pub(crate) fn is_disjoint_from<'c>(
        ...
        checker.check_specialization_pair(db, env, self, other)
    }
...
impl<'c, 'db> TypeRelationChecker<'_, 'c, 'db> {
    ...
    fn materialize_constrained_type_argument(
        ...
        let argument_top = ty.materialize(
            db,
            env,
            MaterializationKind::Top,
            self.materialization_visitor,
        ...
        let viable_constraints = constraints.iter().filter_map(|constraint| {
            let constraint_top = constraint.materialize(
                db,
                env,
                MaterializationKind::Top,
                self.materialization_visitor,
...
impl<'db> ConstraintSetAnalysis<'db> {
    ...
    fn specialization_error(
        ...
        if let SpecializationError::MismatchedBound {
            argument: existing, ..
        }
        | SpecializationError::MismatchedConstraint {
            argument: existing, ..
...
impl<'db, 'c> SpecializationBuilder<'db, 'c> {
        ...
        ) -> Option<PathBoundSolution<'db>>,
    {
        ...
        if generic_context
            .variables(db)
            .any(|typevar| typevar.is_parameter_pack(db) || typevar.is_typevartuple(db))
        {
            return Ok(
        ...
        ) -> Option<PathBoundSolution<'db>>,
    {
        let env = self.env;
        let db = self.db;
        let mapped_ty = match &mut self.types {
            LegacyTypeMappings::Available(types) => types
                .get_mut(&variable.identity(db))
                .map(|accumulator| accumulator.get_or_build(db, env)),
            LegacyTypeMappings::BudgetExceeded => None,
        };
        ...
        match mapped_ty {
            Some(mapped_ty) => {
                let path_bound = PathBound::exact(variable, mapped_ty);
                Some(
                    choose(variable, Some(&path_bound))
                        .and_then(PathBoundSolution::as_type)
                        .unwrap_or(mapped_ty),
                )
            }
            None => choose(variable, None).and_then(PathBoundSolution::as_type),
        }
    }
    ...
    fn intersect_pending_typevar_constraint(&mut self, constraint: Constraint<'db>) {
        ...
        let identity = bound_typevar.identity(db);
        if bound_typevar.is_parameter_pack(db) && !self.paramspec_seen.insert(identity) {
            return;
        }
    ...
    fn insert_pending_type_mapping(
        ...
        let constraint = match variance {
            ...
            TypeVarVariance::Bivariant => {
                self.unconstrained.insert(bound_typevar.identity(db));
                return;
            }
        };
    ...
    fn infer_map_impl(
        ...
        match (formal, actual) {
                ...
                if bound_typevar.is_inferable(db, self.inferable) =>
            {
                ...
                if bound_typevar.typevar(self.db).has_pack_bound(self.db, env) {
            ...
            //
            // The constraint set reasons about a gradual source through its materializations, and
            // a marker's bottom materialization is `Never`. So `Iterable[T]` solved against
            // `list[Divergent]` records `Never ≤ T` and the marker is gone, while the same
            // parameter declared `list[T]` — read structurally below — keeps it. That is the
            // difference between
            //
            // ```python
            // def h(n: int):
            ...
            // `Iterable`, so the round that still has only the marker to go on solves the element
            // to `Never`, and the `Unknown` that follows from it is recorded as the element type
            // of the `[h(n)]` literal — a query of its own, which the return type's early-round
            // discard never revisits.
            //
            // Only a protocol the actual's MRO actually lists can be read this way; an implicitly
            // implemented one still goes through the constraint set below.
            (
                formal @ (Type::NominalInstance(_) | Type::ProtocolInstance(_)),
                Type::NominalInstance(actual_nominal),
            ) if matches!(formal, Type::NominalInstance(_))
                || (matches!(formal, Type::ProtocolInstance(_))
                    && any_over_type(db, self.env, actual, false, |ty| ty.is_divergent())) =>
            {
                let structural_protocol = match formal {
                    Type::ProtocolInstance(formal_protocol) => formal_protocol
