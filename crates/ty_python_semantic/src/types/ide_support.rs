...
use crate::place::{Place, implicit_builtins_symbol_scope, imported_symbol};
...
use crate::types::extensions::{
    applicable_extensions, extension_can_apply, resolve_extension_member,
};
...
pub fn definitions_for_name<'db>(
    ...
    if let Some(receiver) = block_receiver_definitions(model, node, scope, name_str) {
        return receiver;
    }
    ...
    if !builtin_definitions.is_empty() {
        return builtin_definitions;
    }

    implicit_name_definitions(db, &env, model, node, name_str)
}
...
/// answers for, inferring the receiver's scope through the IDE model — see
/// [`definition_resolution::fallback_attribute_definitions`], which is the order
...
fn definitions_for_fallback_attribute<'db>(
    ...
    let access = definition_resolution::AttributeRead {
        file: model.file(),
        scope: scope.to_scope_id(db, model.program_file()),
        receiver: lhs_ty,
        receiver_expr: Some(&attribute.value),
        optional: attribute.optional,
    };
    definition_resolution::fallback_attribute_definitions(
        db,
        &model.program_environment(),
        &access,
...
fn definitions_for_member<'db>(
    ...
    let db = model.db();
    let mut resolved = definition_resolution::definitions_for_attribute(
        db,
        &model.program_environment(),
        lhs_ty,
        name_str,
...
/// basedpython: the fork's callers pass a [`SemanticModel`] rather than a db and environment.
fn definitions_for_attribute_in_class_hierarchy<'db>(
    class_literal: &ClassLiteral<'db>,
    model: &SemanticModel<'db>,
    attribute_name: &str,
) -> Vec<ResolvedDefinition<'db>> {
    definition_resolution::definitions_for_attribute_in_class_hierarchy(
        model.db(),
        &model.program_environment(),
        class_literal,
        attribute_name,
...
pub fn definitions_for_django_lookup_root<'db>(
    ...
    let Some(queried) = django::lookup_call_model(db, env, callee) else {
        return Vec::new();
    };
    ...
    let lookups = django::lookup_expressions(
        ...
        env,
        file,
        scope.to_scope_id(db, db.program_file(file)),
        queried,
        &call.arguments,
    );
...
impl<'db> CallSignatureDetails<'db> {
    fn from_binding(
        ...
        let specialization = binding.merged_specialization(db, env);
...
fn extension_operator_definitions<'db>(
    ...
    let bindings = match operator.resolution.ty.try_call(db, env, arguments) {
        Ok(bindings) => bindings,
...
pub fn inferred_raises<'db>(
    ...
    let raised = crate::types::exceptions::function_raised_exceptions(db, env, literal);
...
pub fn extension_members<'db>(
    ...
    for &extension in applicable_extensions(db, file) {
        ...
        for member in all_end_of_scope_members(db, extension.body_scope(db)) {
            ...
            if let Some(resolution) = resolve_extension_member(db, env, file, receiver, &name) {
...
pub fn overridable_members<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    class: Type<'db>,
) -> Vec<OverridableMember<'db>> {
    let Type::ClassLiteral(class) = class else {
        return Vec::new();
    };
        ...
        .enumerate()
    {
        ...
        for member in all_end_of_scope_members(db, literal.body_scope(db)) {
            ...
            let Place::Defined(defined) = superclass
                .own_class_member(db, env, None, &name)
                .inner
                .place
            else {
                ...
                .map(|overload| {
                    overload
                        .display_with(
                            db,
                            env,
                            crate::types::DisplaySettings::default().disallow_signature_name(),
                        )
                        .to_string()
                })
            else {
...
pub fn trailing_lambda_implicit_parameters<'db>(
    ...
    // lowering writes always declares one. hinting it is only worth doing when the call
    // fills it — a callback invoked as `fn()` leaves it at its `None` default, and naming
    // it would point the reader at a value that never arrives
    let binds_it = !function.parameters.args.is_empty()
        && crate::types::trailing_lambda::trailing_lambda_passes_it(db, callee) != Some(false);
    let it = binds_it
...
pub fn hintable_parameter_type<'db>(
    ...
    let db = model.db();
    let definition = semantic_index(db, db.program_file(model.file())).try_definition(parameter)?;
...
pub fn iterable_element_type<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    ty: Type<'db>,
) -> Option<Type<'db>> {
...
/// `await` would accept.
pub fn is_awaitable<'db>(db: &'db dyn Db, env: &ProgramEnvironment<'db>, ty: Type<'db>) -> bool {
    ty.try_await(db, env).is_ok()
}
...
pub fn no_argument_call_return_type<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    ty: Type<'db>,
) -> Option<Type<'db>> {
    ty.try_call(db, env, &CallArguments::none())
        .ok()
...
pub fn instance_of_class<'db>(
    ...
    let module =
        resolve_module_confident(db, env.resolver_environment(db), &ModuleName::new(module)?)?;
...
/// place.
pub fn callable_needs_arguments<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    ty: Type<'db>,
) -> bool {
    matches!(ty, Type::FunctionLiteral(_) | Type::BoundMethod(_))
        && no_argument_call_return_type(db, env, ty).is_none()
}
...
pub fn template_lookup<'db>(
    ...
    let declares_do_not_call = member
        .member(db, env, "do_not_call_in_templates")
        .place
        .ignore_possibly_undefined()
        .is_some_and(|flag| flag.bool(db, env).is_always_true());

    if declares_do_not_call {
        TemplateLookup::UsesUncalled
    } else if django::refuses_template_call(db, env, receiver, name, member) {
...
pub fn own_class_member_names<'db>(
    db: &'db dyn Db,
    env: &ProgramEnvironment<'db>,
    ty: Type<'db>,
) -> FxIndexSet<Name> {
    ...
    let Some((class, _)) = instance.class(db, env).static_class_literal(db) else {
        return FxIndexSet::default();
    };
