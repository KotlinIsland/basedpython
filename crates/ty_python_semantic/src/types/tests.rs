...
fn a_gradual_member_is_visible_through_a_union_or_an_intersection() {
    ...
    let str_ = KnownClass::Str.to_instance(&db, &db.program_environment());
    ...
    assert!(
        UnionType::from_elements(&db, &db.program_environment(), [unknown, int])
            .has_gradual_member(&db, &db.program_environment())
    );
    assert!(
        IntersectionBuilder::new(&db, &db.program_environment())
    ...
    assert!(
        !UnionType::from_elements(&db, &db.program_environment(), [int, str_])
            .has_gradual_member(&db, &db.program_environment())
    );
    assert!(
        !IntersectionBuilder::new(&db, &db.program_environment())
    ...
    assert!(narrowed.has_gradual_member(&db, &db.program_environment()));

    // and the predicate is about a *member*: a type that is itself gradual has none
    assert!(!unknown.has_gradual_member(&db, &db.program_environment()));

    // a generic argument is not a member — `list[Unknown]` is still every bit a `list`
    assert!(
        !KnownClass::List
            .to_specialized_instance(&db, &db.program_environment(), &[unknown])
            .has_gradual_member(&db, &db.program_environment())
    );
    ...
    let bounded_by = |bound| {
        Type::TypeVar(BoundTypeVarInstance::synthetic_self(
            &db,
            bound,
            BindingContext::Synthetic(db.program()),
        ))
    };
    assert!(bounded_by(unknown).has_gradual_member(&db, &db.program_environment()));
    assert!(
        bounded_by(UnionType::from_elements(
            &db,
            &db.program_environment(),
            [unknown, int]
        ))
    ...
    assert!(!bounded_by(int).has_gradual_member(&db, &db.program_environment()));
}
