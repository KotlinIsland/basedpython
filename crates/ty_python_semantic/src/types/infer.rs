    ...
    fn cycle_normalized(
            ...
            && let DefinitionInferenceExtra::Other(extra) = extra.as_mut()
        {
            if let Some(fluid_creation) = &mut extra.fluid_creation
                ...
                && any_over_type(db, &env, *fluid_creation, false, |ty| ty.is_divergent())
            {
                *fluid_creation = match previous_inference.fluid_creation() {
                    Some(previous_creation) => {
                        fluid_creation.cycle_normalized(db, &env, previous_creation, cycle)
                    }
                    None => fluid_creation.recursive_type_normalized(db, &env, cycle),
                };
            }
