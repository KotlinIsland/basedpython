...
pub(crate) fn synthetic_decorator_target_type<'db>(
    ...
    if let Some(flags) = match name.id.as_str() {
        ...
        let params = crate::types::DataclassParams::from_flags(db, env, flags);
...
impl<'db> OverloadLiteral<'db> {
    ...
    fn is_implicit_overload(self, db: &dyn Db) -> bool {
        // the scope carries the program this query is running under; asking the db for
        // the file's *default* program would resolve the node against a different python
        // version when the same file is checked under several
        let body_scope = self.body_scope(db);
        let module = parsed_module(db, body_scope.python_file(db)).load(db);
        let func_node = body_scope.node(db).expect_function().node(&module);
    ...
    pub(super) fn raw_signature(
        ...
        raw_signature.add_implicit_self_annotation(db, env, || {
            ...
            if method_has_explicit_self || class_is_generic || class_is_fallback {
                ...
                if self.takes_implicit_class_receiver(db) || is_dunder_new {
    ...
    fn overridden_signature(
        ...
        let mentions_typevar =
            |ty: Type<'db>| any_over_type(db, env, ty, false, |ty| matches!(ty, Type::TypeVar(_)));
    ...
    fn overridden_method(
        ...
        let member =
            class_literal.class_member_from_mro(db, env, name, MemberLookupPolicy::default(), mro);
...
fn is_instance_truthiness<'db>(
    ...
    match ty {
        ...
        Type::Overlapping(overlapping) => {
            is_instance_truthiness(db, env, overlapping.value_type(db, env), class)
        }
        Type::Restricted(restricted) => {
            is_instance_truthiness(db, env, restricted.value_type(db), class)
        }
        Type::Deferred(deferred) => {
            is_instance_truthiness(db, env, deferred.reduced(db, env), class)
        }
        Type::Union(..) => {
    ...
    pub(super) fn check_call<'db>(
        ...
        match self {
            ...
            KnownFunction::AssertType => {
                ...
                let diagnostic = if actual_ty.is_spellable(db, env)
