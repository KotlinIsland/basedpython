...
impl ItemCombination {
    ...
    fn combine<'db, I>(self, db: &'db dyn Db, env: &ProgramEnvironment<'db>, types: I) -> Type<'db>
    where
        ...
        for binding in self.iter_flat_mut() {
            ...
            for (overload_index, overload) in binding.matching_overloads_mut() {
                match binding_type {
                    ...
                    {
                        if let Some(model) = django::queryset_or_manager_model(
                            db,
                            env,
                            ...
                            let row = if all_literal {
                                if bound_method.function(db).name(db) == "values_list" {
                                    django::values_list_row_type(
                                        db, env, model, &fields, flat, named,
                                    )
                                } else {
                                    django::values_row_type(db, env, model, &fields)
                                }
                            } else {
                                None
                            };
                            if let Some(row) = row
                                && let Some(refined) =
                                    django::with_queryset_row(db, env, overload.return_ty, row)
                            {
                        ...
                        if dataclass_field_specifiers.contains(&function) =>
                    {
                        ...
                        let frozen = get_argument_type("frozen", false)
                            .is_some_and(|frozen| frozen.bool(db, env).is_always_true());
...
impl<'db> CallableBinding<'db> {
    ...
    pub(crate) fn bake_bound_type_into_overloads(
        ...
        for overload in &mut self.overloads {
            ...
            overload.rebind_self_in_return_type(db, env);
    ...
    fn filter_overloads_using_any_or_unknown(
                    ...
                    .flat_map(move |(matched_argument, argument_types)| {
                        matched_argument.iter().map(move |matched_parameter| {
                            ...
                            let parameter_type = raw_parameter_type.apply_optional_specialization(
                                db,
                                overload.merged_specialization(db, env),
                            );
                            OverloadFilterSlot {
        ...
        if !are_return_types_equivalent_for_all_matching_overloads {
            ...
            self.overload_call_result = Some(OverloadCallResult::Ambiguous(return_type));
        }
    ...
    fn return_type(&self) -> Type<'db> {
        if let Some(overload_call_result) = &self.overload_call_result {
            return match overload_call_result {
                ...
                OverloadCallResult::Ambiguous(return_type) => *return_type,
            };
        }
    ...
    fn report_diagnostics(
        ...
        let describe = |callable_type: Type<'db>| {
            constructed_instance_type
                .and_then(|instance| {
                    CallableDescription::constructed_class(context.db(), env, instance)
                })
...
enum OverloadCallResult<'db> {
    ...
    /// Several overloads remain with non-equivalent return types, because an argument was
...
impl<'a, 'db> ArgumentTypeChecker<'a, 'db> {
    ...
    fn infer_specialization(&mut self, constraints: &ConstraintSetBuilder<'db>) {
        ...
        let maybe_promote = |typevar: BoundTypeVarInstance<'db>, bounds: &PathBound<'db>| {
            ...
            let bound_or_constraints = typevar.typevar(self.db).bound_or_constraints(self.db, env);
            ...
            if typevar.is_declared_invariant_but_never_written(self.db, env) {
                return None;
            }
    ...
    fn infer_argument_constraints<'c>(
        ...
        if let Some(pack_parameter_index) = keyword_pack_parameter {
            for (argument_index, _, argument, argument_types) in self.enumerate_argument_types() {
                for matched_parameter in self.argument_matches[argument_index].iter() {
                    if matched_parameter.index != pack_parameter_index {
                        continue;
                    }
                    ...
                    if let Argument::Keyword(name) = argument {
                        ...
                        keyword_pack_fields.push(
                            Parameter::keyword_only(Name::new(name))
                                .with_annotated_type(field_type.promote(db, env)),
                        );
                    }
                }
            }
        ...
        for relation in self.argument_relations() {
            // the aggregate parameter is solved from all of its keyword arguments at once, below
            if keyword_pack_parameter == Some(relation.matched_parameter.index) {
                continue;
            }
    ...
    fn check_argument_type(
        ...
        // basedpython: the *argument* reading of the specialization — a type variable the call
        // left unsolved stays gradual here rather than becoming `Never`, which in a contravariant
        // parameter position would say the parameter accepts nothing
        ...
        let type_error = if self.constraint_set_errors[argument_index]
            || constructor_receiver
            || (has_starred_annotation && matched_parameter.expected_type.is_none())
            || is_valid_isinstance_target()
        {
            ...
            !key.is_never()
                && argument_type
                    .when_assignable_to(db, self.env, key, constraints, self.inferable_typevars)
                    .is_never_satisfied(db, self.env)
                && key.is_disjoint_from(db, self.env, argument_type)
        } else {
            argument_type
                .when_assignable_to(
                    db,
    ...
    fn keyword_aggregate_kind(&self, parameter: &Parameter<'db>) -> Option<KeywordAggregateKind> {
        ...
        match parameter.annotated_type() {
            ...
            annotated if annotated.is_typed_dict_bounded_typevar(self.db, env) => {
                Some(KeywordAggregateKind::TypedDict)
            }
            _ => None,
        }
...
impl<'db> Binding<'db> {
    ...
    fn rebind_self_in_return_type(&mut self, db: &'db dyn Db, env: &ProgramEnvironment<'db>) {
        if !self.return_ty.contains_self(db, env) {
            return;
        }
        ...
        self.return_ty = self.return_ty.apply_type_mapping(
            db,
            env,
            &TypeMapping::BindSelf(SelfBinding::new(db, env, receiver_self_type, None)),
            TypeContext::default(),
        );
    }
        ...
        Some(generic_context.specialize_recursive(
            ...
            generic_context.variables(db).map(|typevar| {
                ...
                let argument_constraints = argument_specialization
                    ...
                    .map(|ty| ty.promote_in(db, env, file));
    ...
    fn check_types(
        ...
        self.rebind_self_in_return_type(db, env);
    }
    ...
    fn resolve_context_arguments(
        ...
        for error in &mut self.errors {
            ...
            missing.0.retain(|parameter_context| {
                let Some(parameter) = parameters.get(parameter_context.signature_parameter_index)
                else {
                    return true;
                };
    ...
    pub(crate) fn merged_specialization(
        &self,
        db: &'db dyn Db,
        env: &ProgramEnvironment<'db>,
    ) -> Option<Specialization<'db>> {
...
impl<'db> CallableDescription<'db> {
    ...
    /// Rendering a qualified name needs an environment as well as the settings, so the two
    /// travel together: without settings there is no qualified form to render.
    fn new_with_settings(
        db: &'db dyn Db,
        callable_type: Type<'db>,
        settings: Option<(&ProgramEnvironment<'db>, &DisplaySettings<'db>)>,
    ) -> Option<CallableDescription<'db>> {
        fn qualified_function_name<'db>(
            ...
            if let Some((env, settings)) = settings
                && let Some(class) =
                    CallableDescription::defining_class(db, Type::FunctionLiteral(function))
            {
                Cow::Owned(format!(
                    "{}.{}",
                    class.display_with(db, env, settings.clone()),
                    function.name(db)
                ))
            } else {
                Cow::Borrowed(
    ...
    fn constructed_class(
        ...
        let class = instance_type.as_nominal_instance()?.class(db, env);
        Some(CallableDescription {
            kind: Some("class"),
...
impl std::fmt::Display for ParameterContext {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        if let Some(name) = &self.name {
            ...
            write!(f, "{} (the receiver)", self.signature_parameter_index + 1)
        } else {
            ...
            Self::SpecializationError {
                ...
                match error {
                    ...
                    SpecializationError::UnsatisfiedPackBound {
                        ...
                        diag.set_primary_annotation_message(violation.message(
                            context.db(),
                            env,
                            *bound_typevar,
                        ));
