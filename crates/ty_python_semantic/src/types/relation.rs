...
impl<'a, 'c, 'db> TypeRelationChecker<'a, 'c, 'db> {
    ...
    pub(super) fn check_type_pair(
            ...
            && (source.is_deferred() || union.elements(db).iter().copied().any(Type::is_deferred))
        {
            ...
            if distributed.is_always_satisfied(db, env) {
                return distributed;
            }
        }
        ...
        match (source, target) {
            ...
            (_, Type::Restricted(target_restricted)) => {
                ...
                if restriction_admits(db, env, target_restricted.modifier(db), inner, source) {
            ...
            (_, Type::Deferred(target_deferred)) if target_deferred.is_checked(db) => {
                let names_another_value =
                    !any_over_type(db, env, source, false, |ty| ty.is_dynamic())
                ...
                if names_another_value {
                    ...
                    self.check_type_pair(
                        db,
                        source.reduce_deferred(db, env),
                        target_deferred.reduced(db, env),
                    )
                }
            }
                ...
                if source_deferred.is_checked_arithmetic(db) =>
            {
                match LinearForm::same_value(db, source, target) {
                    ...
                    Some(false) if !any_over_type(db, env, source, false, |ty| ty.is_dynamic()) => {
                        self.never()
                    }
                    _ => self.check_type_pair(db, source_deferred.reduced(db, env), target),
                }
            }

            (Type::Deferred(source_deferred), _) => {
                self.check_type_pair(db, source_deferred.reduced(db, env), target)
            }

            (_, Type::Deferred(target_deferred)) => {
                self.check_type_pair(db, source, target_deferred.reduced(db, env))
            }
            ...
            (_, Type::UnsafeUnion(target_unsafe_union)) => match self.relation {
                TypeRelation::Subtyping | TypeRelation::SubtypingAssuming => self.never(),
                TypeRelation::Assignability => {
                    self.check_type_pair(db, source, target_unsafe_union.to_union(db, env))
                }
            ...
            (_, Type::TypeVar(bound_typevar))
                if self.is_eager_assignability()
                    && !bound_typevar.is_inferable(db, self.inferable)
                    && crate::types::inferred_signature::gradual_hole(db, env, target)
                        .is_some() =>
            {
                self.always()
            }
            ...
            // An unbounded type variable ranges over every `object`, so the rule that applies
            // to `object` itself applies here: for *subtyping* it says nothing, because the
            // variable could be specialized to a type that does not implement the target, but
            // for assignability the members are inspected rather than refused outright. Without
            // this a `T` could not fill a `Hashable` parameter, which is what `dict[T, V]` asks
            // of its key.
            (Type::TypeVar(bound_typevar), _)
            ...
            (_, Type::KnownInstance(KnownInstanceType::WrappedOptional(_))) => {
                self.check_type_pair(db, source, Type::none(db, env))
            }
            ...
            (Type::NominalInstance(source_i), Type::NominalInstance(target_i)) => self
                .with_recursion_guard(db, source, target, || {
                    self.check_nominal_instance_pair(db, env, source_i, target_i)
                }),
...
impl<'a, 'c, 'db> DisjointnessChecker<'a, 'c, 'db> {
    ...
    fn check_type_pair_impl(
        ...
        match (left, right) {
            ...
            (Type::Deferred(deferred), _) => {
                self.check_type_pair(db, deferred.reduced(db, env), right)
            }

            (_, Type::Deferred(deferred)) => {
                self.check_type_pair(db, left, deferred.reduced(db, env))
            }
            ...
            | (Type::NominalInstance(instance), Type::LiteralValue(literal)) => {
                nontrivial_check(self, || {
                    let positive_relation_holds = match literal.kind() {
                        ...
                        LiteralValueTypeKind::Float(_) => KnownClass::Float.when_subclass_of(
                            db,
                            env,
                            instance.class(db, env),
                            self.constraints,
                        ...
                        LiteralValueTypeKind::String(value) => {
                            ...
                            literal_class.when_subclass_of(
                                db,
                                env,
                                instance.class(db, env),
                                self.constraints,
