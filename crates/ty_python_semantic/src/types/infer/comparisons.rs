...
fn infer_binary_type_comparison_inner<'db>(
    ...
    let comparison_result = match (left, right) {
        ...
        (Type::Union(_), other) if matches!(op, NonIdentityOperator::Membership(_)) => {
            let NonIdentityOperator::Membership(membership_op) = op else {
                unreachable!("the guard above matched a membership operator")
            };
