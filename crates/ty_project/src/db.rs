...
pub(crate) mod testing {
    ...
    impl TestDb {
        ...
        fn init_program_inner(
            ...
            // the project has to carry these too: `Project::program` rebuilds the program
            // from the project's own settings, so a query that asks the project rather
            // than a file would otherwise see no site-packages at all
            Program::from_settings(self, &settings);
...
fn script_dependency_manifest(db: &dyn Db, file: File) -> Option<DependencyManifest> {
    let tag = crate::script::script_tag(db, file)?;
    let path = file.path(db).as_system_path()?;
    PyProject::from_toml_str_without_spans(
        tag.metadata(),
        ValueSource::File(Arc::new(path.to_path_buf())),
    )
    .ok()?
    .dependency_manifest()
}
