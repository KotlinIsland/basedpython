...
pub(crate) fn derive_impl(input: DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
    ...
    let mut name_match_arms = quote!(
        Self::Ruff => "Ruff-specific rules",
        Self::Numpy => "NumPy-specific rules",
        Self::Basedpython => "basedpython-specific rules",
    );
    let mut url_match_arms =
        quote!(Self::Ruff => None, Self::Numpy => None, Self::Basedpython => None, );
    ...
    for variant in variants {
        ...
        // these three name a body of rules of our own rather than an upstream
        // linter, so they have no name to parse out of the doc comment and no
        // project to link to
        if !matches!(
            variant_ident.to_string().as_str(),
            "Ruff" | "Numpy" | "Basedpython"
        ) {
