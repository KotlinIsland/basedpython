...
def _by_code(origin):
    ...
    python answers each from the code object. So the code object is compiled under
    ...
    file's, so every node spans the whole of its `.by` line instead: python marks
    no part of a line a node covers, and reads what it can from the `.by` text.
    ...
    with open(origin, "rb") as handle:
        tree = ast.parse(handle.read(), origin)
    ...
    for node in ast.walk(tree):
        if "lineno" in node._attributes:
            node.lineno = node.end_lineno = by_line(node.lineno)
            node.col_offset = 0
            node.end_col_offset = widths[node.lineno - 1] if node.lineno else 0
    return compile(tree, by_path, "exec", dont_inherit=True)
