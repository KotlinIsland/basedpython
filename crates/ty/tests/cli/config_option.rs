...
fn cli_config_args_invalid_option() -> anyhow::Result<()> {
    ...
    1 | bad-option=true
      | ^^^^^^^^^^
    unknown field `bad-option`, expected one of `type-checking-preset`, `environment`, `src`, `rules`, `terminal`, `analysis`, `experimental`, `run`, `build`, `lowering`, `compile`, `editor`, `overrides`
...
fn invalid_configuration_file() -> anyhow::Result<()> {
    ...
    by failed
      ...
      Cause: <temp_dir>/ty.toml is not a valid configuration file
