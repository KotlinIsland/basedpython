...
# a rung's summary, written into `$OUT` as well as to the terminal
#
# a waiter watching the output file for `walked:` has nothing else to watch: the rows say
# nothing about whether the walk has finished, and a summary that only reached stdout went
# into whichever log the rung was started under. each line lands in `$OUT` behind a `# `,
# which no module name starts with, so `sweep_end` counts it as no module and a reader
# picking rows by their tab-separated verdict never sees it. it has to be written before
# `sweep_end`, whose completion marker vouches for every line the file holds
sweep_summary() {
  local line
...
sweep_end() {
  ...
  # a summary line is not a module — see `sweep_summary`
  seen=$(grep -v '^#' "$OUT" | cut -f1 | LC_ALL=C sort -u | grep -c .)
  if [ "$walked" -eq 0 ]; then
...
# which names two builds of `by.h` disagree about
#
# prints `same` when nothing a module could reach has moved, `unscopable` when the change
# cannot be attributed to particular names, or those names joined by `|` for `grep -owE`.
# takes the two headers, and says on stderr why a change is unscopable
#
# a name is scoped *out* of a walk only when the evidence says so plainly, because getting
# this wrong in the narrowing direction hides a real divergence: a module reported `same`
# is one the behavioural rungs never run. `headerscope.py` compares the two headers
# definition by definition rather than line by line, which is what lets a purely additive
# change — a new helper, whose body is mostly lines naming nothing — scope to the names it
# adds, while a changed macro or a changed struct layout, which every definition expanding
# or holding one depends on, is still unscopable. its docstring has the rules, and
# `test_headerscope.py` a fixture for each
sweep_header_scope() {
  "$PY" "$(dirname "${BASH_SOURCE[0]}")/headerscope.py" "$1" "$2"
}
...
# a compiled class publishes its methods as a `by.method_descriptor`, which binds as a
# function does but is no type python has a name for, so it is known by its own
def _compiled_method(kind):
    return getattr(kind, '__module__', None) == 'by' and kind.__qualname__ == 'method_descriptor'
...
class Canon:
    ...
    def _render(self, value, depth):
        ...
        if isinstance(value, _CALLABLE) or _compiled_method(kind):
