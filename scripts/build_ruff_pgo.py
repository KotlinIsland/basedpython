...
# basedpython ships the linter as `buff`, so the `ruff` package's binary is not called `ruff`
BINARY_STEM = "buff"
...
def main():
    ...
    binary_name = f"{BINARY_STEM}.exe" if "windows" in target else BINARY_STEM
...
def run_git_with_retry(command: list[str], *, environment: dict[str, str]):
